...
if ! docker inspect "$CONTAINER_NAME" >/dev/null 2>&1; then
  ...
  echo "Default behavior prefers the RTX 5000 Ada GPU when present."
  echo "Example override for 2 GPUs: NWM_GPU_REQUEST='device=0,1' ./scripts/docker/nwm-start.sh"
  exit 1
fi
